...
contract NFTMarketTest is Test {
    ...
    function setUp() public {
        ...
        // proxy
        bytes memory erc20InitCallData = abi.encodeWithSignature("initialize(string,string,address)",
        "TUToken", "TUTK", owner);
        proxyToken = new ERC1967Proxy(address(payToken), erc20InitCallData);
        // nft v1 impl
        nft = new TNFTUpgradeable();
        // proxy
        bytes memory nftInitCallData = abi.encodeWithSelector(TNFTUpgradeable.initialize.selector,
        "TerryUNFT", "TUNFT", owner);
...
//        "initialize(string memory marketName_, address paymentToken_, address nft_, address owner)"
        bytes memory marketInitCallData = abi.encodeWithSelector(TerryUpgradeableNFTMarket.initialize.selector,
            "TUMarket", address(payToken), address(nft), owner);
...
//            "TUMarketV2", address(payToken), address(nft), owner);
        bytes memory marketV2UpgradeCallData = abi.encodeWithSelector(TerryUpgradeableNFTMarketV2.upgradeVersion.selector, 2);
